export class ImageLib {
    ...
    drawTarget(target, radius) {
        ...
        let leafBaseX = target.x + 1;
        let leafBaseY = target.y - radius - 0.7 * radius + 5;

        this.create_line(leafBaseX, leafBaseY, leafBaseX + 10, leafBaseY - 5, { r: 0, g: 100, b: 0 });
        this.create_line(leafBaseX + 10, leafBaseY - 5, leafBaseX, leafBaseY - 10, { r: 0, g: 100, b: 0 });
    }
        ...
        document.addEventListener("keydown", (event) => {
            if (!isReversing) {
                switch (event.key) {
                    case "1":
                        if (direction.y == 0) {
                            direction = { x: 0, y: -radius }; // Atas
                        }
                        break;
                    case "2":
                        if (direction.y == 0) {
                            direction = { x: 0, y: radius }; // Bawah
                        }
                        break;
                    case "-":
                        if (direction.x == 0) {
                            direction = { x: -radius, y: 0 }; // Kiri
                        }
                        break;
                    case "=":
                        if (direction.x == 0) {
                            direction = { x: radius, y: 0 }; // Kanan
                        }
                        break;
                }
            }
